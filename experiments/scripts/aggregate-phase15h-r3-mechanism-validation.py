#!/usr/bin/env python3
"""Aggregate Phase 15H-R3 TL-vs-Volume mechanism validation outputs."""
...
SUMMARY_OUT = PROCESSED_DIR / "phase15h-r3-mechanism-summary.csv"
QUALITY_OUT = PROCESSED_DIR / "phase15h-r3-mechanism-quality-report.csv"
SCHED_OUT = PROCESSED_DIR / "phase15h-r3-scheduling-diagnostics.csv"
CLOSENESS_OUT = PROCESSED_DIR / "phase15h-r3-oracle-closeness.csv"

NAME_RE = re.compile(
    r"^phase15h-r3-(?P<scenario>.+)-"
...
def aggregate_run(summary_path):
    ...
    if sched_summary is None:
        sched_summary = {key: float("nan") for key in SUMMARY_FIELDS if key.startswith("avg_")}
...
def quality_checks(summary_rows):
    ...
    summaries = {path.stem: path for path in RAW_DIR.glob("phase15h-r3-*.csv") if not path.stem.endswith("-flows") and not path.stem.endswith("-scheduling")}
...
def main():
    summary_rows = []
    scheduling_rows = []
    for path in sorted(RAW_DIR.glob("phase15h-r3-*.csv")):
