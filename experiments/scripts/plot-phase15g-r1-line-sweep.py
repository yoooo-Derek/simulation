#!/usr/bin/env python3
"""Plot Phase 15G-R1 verification line-sweep figures."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--aggregate",
        default="results/processed/phase15g-r1-line-summary-aggregate.csv",
        type=Path,
    )
    parser.add_argument("--output-dir", default="results/figures/phase15g-r1", type=Path)
...
def load_rows(path: Path) -> list[dict[str, str]]:
    if not path.exists():
        raise FileNotFoundError(
            f"{path} does not exist; run aggregate-phase15g-r1-line-sweep.py first"
        )
    ...
    for scenario in sorted({row["scenario"] for row in rows}):
        ...
        for metric, ylabel, suffix in PLOTS:
            ...
            for scheme in SCHEMES:
                scheme_rows = sorted(
                    by_scheme.get(scheme, []),
                    key=lambda item: to_float(item["offered_load_factor"]),
                )
                if not scheme_rows:
                    continue
                ...
                has_data = has_data or any(value == value for value in y)
            ...
            plt.legend()
            plt.tight_layout()
            output = args.output_dir / f"phase15g-r1-{scenario_slug(scenario)}-{suffix}-vs-load.png"
