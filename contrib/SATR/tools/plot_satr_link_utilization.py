#!/usr/bin/env python3
"""Plot SATR average link utilization curves from performance-summary.csv."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a paper-style link utilization line chart from SATR performance CSV data."
    )
    parser.add_argument(
        "--csv",
        type=Path,
    ...
    parser.add_argument(
        "--out-prefix",
        default="link_utilization_vs_offered_load",
        help="Output filename prefix. Default: link_utilization_vs_offered_load",
    )
    parser.add_argument(
        "--metric",
        default="avgLinkUtilization",
        help="Metric column to plot. Default: avgLinkUtilization",
    )
    parser.add_argument(
        "--ylabel",
        default="Average link utilization",
        help='Y-axis label. Default: "Average link utilization"',
    )
    parser.add_argument(
        "--convert-percent",
        action="store_true",
    ...
    parser.add_argument(
        "--panel-label",
        default=None,
        help='Optional panel label drawn below the top axis border, e.g. "(c)".',
    )
    parser.add_argument(
        "--pdf-only",
        action="store_true",
...
def main() -> None:
    args = parse_args()
    df = load_data(args.csv, args.metric, args.convert_percent)
    if df.empty:
