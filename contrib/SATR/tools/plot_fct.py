#!/usr/bin/env python3
"""Plot SATR metric curves from performance-summary.csv.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a paper-style line chart from SATR performance CSV data."
    )
    parser.add_argument(
        "--csv",
        type=Path,
    ...
    parser.add_argument(
        "--panel-label",
        default=None,
        help='Optional panel label drawn below the top axis border, e.g. "(a)".',
    )
    parser.add_argument(
        "--pdf-only",
        action="store_true",
