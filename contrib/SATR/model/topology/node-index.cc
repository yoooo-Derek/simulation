...
NodeIndex::SetLeafNodes(uint32_t groupId, const NodeContainer& leaves)
{
    ...
    {
        throw std::out_of_range("SATR group leaf index is out of range");
    }
...
NodeIndex::SetGroupSpineNodes(uint32_t groupId, const NodeContainer& spines)
{
    ...
    {
        throw std::out_of_range("SATR group spine index is out of range");
    }
...
NodeIndex::SetServerIpv4Address(uint32_t torId, uint32_t serverId, Ipv4Address address)
{
    ...
    {
        throw std::out_of_range("SATR server address index is out of range");
    }
...
NodeIndex::SetOcsServerIpv4Address(uint32_t torId, uint32_t serverId, Ipv4Address address)
{
    ...
    {
        throw std::out_of_range("SATR OCS server address index is out of range");
    }
...
NodeIndex::SetServerLinkInfo(uint32_t torId, uint32_t serverId, const ServerLinkInfo& linkInfo)
{
    ...
    {
        throw std::out_of_range("SATR server link index is out of range");
    }
...
NodeIndex::SetTorIngressDevice(uint32_t torId, uint32_t serverId, Ptr<NetDevice> device)
{
    ...
    {
        throw std::out_of_range("SATR ToR ingress device index is out of range");
    }
...
NodeIndex::AddTorSpineLink(const TorSpineLinkInfo& linkInfo)
{
    ...
    {
        throw std::out_of_range("SATR EPS ToR-spine link index is out of range");
    }
...
NodeIndex::AddLeafSpineLink(const LeafSpineLinkInfo& linkInfo)
{
    ...
    {
        throw std::out_of_range("SATR leaf-spine link index is out of range");
    }
...
NodeIndex::AddOpticalAccessLink(const OpticalAccessLinkInfo& linkInfo)
{
    ...
    {
        throw std::out_of_range("SATR optical access link index is out of range");
    }
...
NodeIndex::AddOcsLink(const OcsLinkInfo& linkInfo)
{
    ...
    {
        throw std::out_of_range("SATR OCS link ToR index is out of range");
    }
    if (m_memsCount > 0)
    {
        ...
        {
            throw std::out_of_range("SATR active optical circuit is not backed by MEMS access");
        }
    }
...
NodeIndex::AddInterPodElectricalLink(const InterPodElectricalLinkInfo& linkInfo)
{
    ...
    {
        throw std::out_of_range("SATR inter-pod electrical link index is out of range");
    }
...
NodeIndex::GetServer(uint32_t torId, uint32_t serverId) const
{
    ...
    {
        throw std::out_of_range("SATR server index is out of range");
    }
...
NodeIndex::GetTor(uint32_t torId) const
{
    ...
    {
        throw std::out_of_range("SATR ToR index is out of range");
    }
...
NodeIndex::GetSpine(uint32_t spineId) const
{
    ...
    {
        throw std::out_of_range("SATR spine index is out of range");
    }
...
NodeIndex::GetLeaf(uint32_t groupId, uint32_t leafId) const
{
    ...
    {
        throw std::out_of_range("SATR leaf index is out of range");
    }
...
NodeIndex::GetMems(uint32_t memsId) const
{
    ...
    {
        throw std::out_of_range("SATR MEMS index is out of range");
    }
...
NodeIndex::GetTorSpineLink(uint32_t torId, uint32_t spineId) const
{
    ...
    {
        throw std::out_of_range("SATR EPS ToR-spine link does not exist");
    }
    return match->second;
}
...
NodeIndex::GetLeafSpineLink(uint32_t groupId, uint32_t leafId, uint32_t spineId) const
{
    ...
    {
        throw std::out_of_range("SATR leaf-spine link does not exist");
    }
    return match->second;
}
...
NodeIndex::GetOpticalAccessLink(uint32_t groupId, uint32_t spineId, uint32_t memsId) const
{
    ...
    {
        throw std::out_of_range("SATR optical access link does not exist");
    }
    return match->second;
}
...
NodeIndex::GetOcsLink(uint32_t torA, uint32_t torB) const
{
    ...
    if (match == m_ocsLinks.end() || match->second.empty())
    {
        throw std::out_of_range("SATR OCS link does not exist for ToR pair");
    }
    return match->second.front();
}

bool
NodeIndex::HasOcsLink(uint32_t torA, uint32_t torB, uint32_t memsId) const
{
    const auto match = m_ocsLinks.find(NormalizePair(torA, torB));
    ...
    return std::any_of(match->second.begin(),
                       match->second.end(),
                       [memsId](const OcsLinkInfo& link) {
                           return link.memsId == memsId;
                       });
}

NodeIndex::OcsLinkInfo
NodeIndex::GetOcsLink(uint32_t torA, uint32_t torB, uint32_t memsId) const
{
    const auto links = GetOcsLinks(torA, torB);
    const auto match = std::find_if(links.begin(),
                                    links.end(),
                                    [memsId](const OcsLinkInfo& link) {
                                        return link.memsId == memsId;
                                    });
    if (match == links.end())
    {
        throw std::out_of_range("SATR OCS link does not exist for ToR pair and MEMS");
    }
    return *match;
}
...
NodeIndex::GetOcsLinks() const
{
    ...
    for (const auto& [key, pairLinks] : m_ocsLinks)
    {
        links.insert(links.end(), pairLinks.begin(), pairLinks.end());
    }
    return links;
}

std::vector<NodeIndex::OcsLinkInfo>
NodeIndex::GetOcsLinks(uint32_t torA, uint32_t torB) const
{
    const auto match = m_ocsLinks.find(NormalizePair(torA, torB));
    if (match == m_ocsLinks.end())
    {
...
NodeIndex::GetOcsPeerAddress(uint32_t sourceTor, uint32_t destinationTor) const
{
    ...
    throw std::out_of_range("SATR OCS peer address requested for non-endpoint ToR");
}
...
NodeIndex::GetOcsInterfaceIndex(uint32_t sourceTor, uint32_t destinationTor) const
{
    ...
    throw std::out_of_range("SATR OCS interface requested for non-endpoint ToR");
}
...
{
    uint32_t count = 0;
    for (const auto& [key, links] : m_ocsLinks)
    {
        count += static_cast<uint32_t>(links.size());
    }
    return count;
}
...
NodeIndex::HasPureElectricalPath(uint32_t sourceGroup, uint32_t destinationGroup) const
{
    ...
    {
        throw std::out_of_range("SATR group index is out of range");
    }
