#!/usr/bin/env python3
"""Plot SMTRA receiver throughput curves from performance-summary.csv."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a paper-style throughput line chart from SMTRA performance CSV data."
    )
    parser.add_argument(
        "--csv",
        type=Path,
    ...
    parser.add_argument(
        "--ylabel",
        default="Receiver throughput (Gbps)",
        help='Y-axis label. Default: "Receiver throughput (Gbps)"',
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
